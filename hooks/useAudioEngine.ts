...
class ScratchProcessor extends AudioWorkletProcessor {
        ...
        this.port.onmessage = ({ data }) => {
            if (data.type === 'init') {
                ...
                if (this._isScratching) {
                    let deltaAngle = data.angle - this._initialScratchAngle;
                    if (deltaAngle > Math.PI) deltaAngle -= 2 * Math.PI;
                    if (deltaAngle < -Math.PI) deltaAngle += 2 * Math.PI;

                    const timeDelta = deltaAngle * this._radiansToSeconds;
                    const sampleDelta = timeDelta * sampleRate;
                    const newPlayhead = this._initialScratchPlayhead + sampleDelta;
                    this._scratchTargetPlayhead = newPlayhead;
                }
    ...
    process(inputs, outputs) {
        ...
        const bufferLength = this._buffer[0]?.length || 0;
        ...
        if (this._isScratching) {
            endPlayhead = this._scratchTargetPlayhead;
        } else {
            endPlayhead = startPlayhead + (this._playbackRate * blockSize);
        }

        const rate = (endPlayhead - startPlayhead) / blockSize;
